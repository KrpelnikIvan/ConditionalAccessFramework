...
| `REPLACE-WITH-NAMED-LOCATION-ID` | Country named location referenced by CA-Global-120 | ID of the `Untrusted Countries` named location (country blocklist). Create it before import |
...
| 8 | Licensing: Entra ID P1 for the baseline, P2 for the risk-based policies, Entra Agent ID or Workload Identities licensing for the Agent scope | CA-Global-070, CA-Global-080, CA-Global-140, CA-Global-150, CA-Agent-010 |
| 9 | Service account exclusions are pre-wired: the service account token is already excluded from the interactive user policies it cannot satisfy (MFA, compliant device). Replace the token with the service account or its group and document the compensating controls | CA-Internal-010, CA-Internal-030, CA-Global-090 |
...
**Agent identities.** CA-Agent-010 blocks agent service principals with high risk. Risk evaluation for workload and agent identities requires the corresponding licensing; without it the condition never matches. A break-glass agent can be added to the excluded service principals if the environment has one.
...
2. Replace all `REPLACE-WITH-*` tokens (see Placeholders above). Use dedicated exclusion groups per policy rather than individual users where possible.
